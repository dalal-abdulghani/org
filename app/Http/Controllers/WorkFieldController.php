...
class WorkFieldController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $coverImage = $request->file('cover_image')->move(public_path('images/work_field_images'), $request->file('cover_image')->getClientOriginalName());

        WorkField::create([
            'name' => $request->name,
            'cover_image' => 'images/work_field_images/' . $request->file('cover_image')->getClientOriginalName(),
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('cover_image')) {
            if (file_exists(public_path($workField->cover_image))) {
                unlink(public_path($workField->cover_image));
            }

            $coverImage = $request->file('cover_image')->move(public_path('images/work_field_images'), $request->file('cover_image')->getClientOriginalName());
            $workField->cover_image = 'images/work_field_images/' . $request->file('cover_image')->getClientOriginalName();
        }
    ...
    public function destroy($id)
    {
        ...
        if ($workField->cover_image && Storage::disk('public')->exists($workField->cover_image)) {
            Storage::disk('public')->delete($workField->cover_image);
        }
